...
// 1日のMP上限・今日のミッションの状態をレスポンスに含めるための共通ヘルパー。
// students行の日付が今日(JST)でなければ、まだ書き戻されていないだけで実質
// 「今日はまだ0件」なので、そのように正規化して返す(handleSyncPointsの
// マージロジックと同じ考え方)。ログイン・再開時にこれを返すことで、他の端末が
// 先に今日の上限まで稼いでいた場合でも、新しく開いた端末がそれを知らずに
// 上限を二重に使ってしまう不具合(生徒からの実報告あり)を防ぐ。
function todayScopedDailyFields(row) {
  const today = dateKeyTokyo(new Date());
  const pointsTodayValid = row.pointsDate === today;
  const missionTodayValid = row.missionDate === today;
  return {
    pointsDate: today,
    pointsTodayCalc: pointsTodayValid ? row.pointsTodayCalc : 0,
    pointsTodayWord: pointsTodayValid ? row.pointsTodayWord : 0,
    pointsTodayBonus: pointsTodayValid ? row.pointsTodayBonus : 0,
    missionDate: today,
    missionCorrect: missionTodayValid ? row.missionCorrect : 0,
    missionClaimed: missionTodayValid ? row.missionClaimed : false,
  };
}

module.exports = { sha256Hex, dateKeyTokyo, monthKeyTokyo, mondayKeyTokyo, parseJsonCell, todayScopedDailyFields };
