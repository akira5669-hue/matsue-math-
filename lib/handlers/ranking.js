...
async function handleRanking(body) {
  ...
  const data = await sql().query('SELECT id, level, exp, grade FROM students', []);
  const rows = data
    .filter((d) => d.id)
    .map((d) => ({ id: d.id, level: Number(d.level) || 1, exp: Number(d.exp) || 0, grade: d.grade || '' }))
...
async function handleRankingGrade(body) {
  ...
  const data = await sql().query('SELECT id, level, exp, grade FROM students WHERE grade = $1', [myGrade]);
  const rows = data
    .filter((d) => d.id)
    .map((d) => ({ id: d.id, level: Number(d.level) || 1, exp: Number(d.exp) || 0, grade: d.grade || '' }))
    .sort((a, b) => (b.level !== a.level ? b.level - a.level : b.exp - a.exp));
...
module.exports = { handleRanking, handleRankingGrade, handleRankingToday, handleRankingPoints, handleRankingHp, handleChallengeRanking };
