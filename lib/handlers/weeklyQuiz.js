...
async function handleWeeklyQuizGet(body) {
  ...
  if (WEEKLY_QUIZ.weekKey !== currentWeek) return { ok: true, available: false, debugWeekKey: WEEKLY_QUIZ.weekKey, debugCurrentWeek: currentWeek, debugGrade: row.grade };
  const quiz = WEEKLY_QUIZ.byGrade[row.grade];
  if (!quiz) return { ok: true, available: false, debugNoQuizForGrade: row.grade, debugGradeKeys: Object.keys(WEEKLY_QUIZ.byGrade) };
