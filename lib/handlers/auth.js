...
const { sha256Hex, dateKeyTokyo, todayScopedDailyFields } = require('../util');
...
// 通信が不安定な環境で登録フォームを連打/再送信すると、その都度新しいIDが
// 量産されてしまう事故が多発した(例: 同一生徒名義で10分間に10件登録)。
// confirmDuplicateが立っていない1回目の送信では、同姓同名(同学年)が既に
// いないか確認し、いれば作成せずにduplicate_nameを返す(IDそのものは
// 他の生徒のプライバシーのため教えない。先生に確認してもらう想定)。
// それでも新規登録したい場合(本当に同姓同名の別の生徒等)は、confirmDuplicate:
// trueを付けて再送信すれば作成される。
async function handleRegister(body) {
  ...
  if (!confirmDuplicate) {
    const existing = await sql().query('SELECT id FROM students WHERE name = $1 AND grade = $2 LIMIT 1', [name, grade]);
    if (existing.length) return { ok: false, error: 'duplicate_name' };
  }
