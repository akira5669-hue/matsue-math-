...
// workMeetingPassage6(仕事算・旅人算・通過算)は意図的にコンプリートボーナス対象
// から除外している。この単元を解かなくても(黒帯にしなくても)、小6/中1の
// コンプリートボーナス判定は他の単元だけで成立する。
const CATEGORY_IDS_GRADE6_ = ['fracMulDiv6', 'symmetry6', 'fracDecIntMulDiv6', 'fracWordProblem6', 'ratioWordProblem6', 'ratio6', 'scale6', 'dataValues6', 'arrangeCombine6', 'patternRelation6', 'circleArea6', 'circleSector6', 'speedFrac6', 'prismVolume6'];
...
// 理科の単元コンプリートボーナス(2026-09-18〜2027-01-31)。算数・数学と違い、
...
const SCIENCE_BONUS_DEADLINE_ = '2027-01-31';
...
async function handleSyncPoints(body) {
  ...
  // コンプリートボーナスの判定は、category_ranks(catStats由来。この機能を追加した
  // 時点からローカルで新しく数え始めた値で、生徒によっては本来もっと前から解いて
  // いた単元でも0からカウントされてしまう)を一切信用せず、必ずrecordsテーブルの
  // 生涯累積(=学習記録画面と同じデータ)を直接集計して判定する。
  async function fetchCumulativeCategoryStats_(categoryIds) {
    if (!categoryIds || categoryIds.length === 0) return {};
    const rows = await sql().query(
      `SELECT category, COUNT(*)::int AS total, SUM(CASE WHEN correct THEN 1 ELSE 0 END)::int AS correct
       FROM records WHERE student_id = $1 AND category = ANY($2::text[]) GROUP BY category`,
      [id, categoryIds]
    );
    const map = {};
    rows.forEach((r) => { map[r.category] = { total: Number(r.total) || 0, correct: Number(r.correct) || 0 }; });
    return map;
  }
  function meetsBlackBelt_(stats, minTotal) {
    const total = stats ? stats.total : 0;
    const correct = stats ? stats.correct : 0;
    return total >= minTotal && (total > 0 ? correct / total : 0) >= 0.8;
  }
  // 算数・数学のコンプリートボーナス：自分の学年に対応する単元セット(学年+1つ下の
  // 学年、全てCATEGORY_IDS_*)が全て黒帯(累計100問以上・正答率80%以上)に到達して
  // いて、期限内、かつまだ受け取っていなければ1回だけ500MP付与する。
  ...
  if (mathBonusReq && !row.completionBonus[effectiveGrade] && dateKeyTokyo(new Date()) <= mathBonusReq.deadline) {
    const cumulative = await fetchCumulativeCategoryStats_(mathBonusReq.ids);
    const allBlackBelt = mathBonusReq.ids.every((catId) => meetsBlackBelt_(cumulative[catId], 100));
    if (allBlackBelt) {
      completionBonusesAwarded.push({ key: effectiveGrade, label: effectiveGrade + '算数・数学', mp: COMPLETION_BONUS_MP });
    }
  }
  // 理科のコンプリートボーナス：2027-01-31まで。学年の階層内(小学:小4〜小6、
  // 中学:中1〜中3)で、自分の学年以下の各学年の理科を個別に(重複して)申請
  // できる。金額は学年ごとに固定(SCIENCE_TIER_DEFS_)。黒帯は累計50問以上・
  // 正答率80%以上。
  if (dateKeyTokyo(new Date()) <= SCIENCE_BONUS_DEADLINE_) {
    for (const tierKey of (SCIENCE_ELIGIBLE_TIERS_BY_GRADE_[effectiveGrade] || [])) {
      if (row.completionBonus[tierKey]) continue;
      const tierDef = SCIENCE_TIER_DEFS_[tierKey];
      if (!tierDef) continue;
      const cumulative = await fetchCumulativeCategoryStats_(tierDef.ids);
      const allBlackBelt = tierDef.ids.every((catId) => meetsBlackBelt_(cumulative[catId], 50));
  ...
  // ログイン成功時の+3HPボーナス(app.jsのHP_LOGIN_GATE_BONUS_)は1日1回まで。
  // missionClaimedと同じ考え方で「今日どちらかの端末で既に受け取っていれば
  // 受け取り済み」としてORマージし、結果を返して端末間で揃える(以前はここに
  // 制限が無く、ログアウト→ログインを繰り返すだけでHPを際限なく稼げていた)。
  const localHpBonusClaimedToday = body.hpLoginBonusDate === today;
  const serverHpBonusClaimedToday = row.hpLoginBonusDate === today;
  const mergedHpLoginBonusDate = (localHpBonusClaimedToday || serverHpBonusClaimedToday) ? today : (row.hpLoginBonusDate || null);
  if (mergedHpLoginBonusDate !== row.hpLoginBonusDate) {
    set('hp_login_bonus_date', mergedHpLoginBonusDate);
  }
  ...
  // 富士登山(2026年10月限定)の山頂到達フラグ・勇者の剣の所持数。山頂到達は
  // 一度trueになったら戻らない実績フラグなのでOR(どちらか一方でも達成済みなら
  // 達成済み)でマージする。剣の所持数はironWallCharges等と同じくクライアントを
  // 信頼してそのままSETする(0または1、1回使うと壊れて消える消費アイテム)。
  if (body.fujiSummitReached !== undefined) {
    const mergedFujiSummit = !!row.fujiSummitReached || !!body.fujiSummitReached;
    if (mergedFujiSummit !== !!row.fujiSummitReached) set('fuji_summit_reached', mergedFujiSummit);
  }
  if (body.yushaSwordCount !== undefined) {
    set('yusha_sword_count', Math.max(0, Math.min(1, Math.floor(Number(body.yushaSwordCount)) || 0)));
  }
  if (body.yushaSwordObtained !== undefined) {
    const mergedYushaSwordObtained = !!row.yushaSwordObtained || !!body.yushaSwordObtained;
    if (mergedYushaSwordObtained !== !!row.yushaSwordObtained) set('yusha_sword_obtained', mergedYushaSwordObtained);
  }
  // 富士登山の到達合目(fujiStation)は進んでいる方(大きい方)を採用する。合目が
  // 進んでいれば今の区間の連続正解数・タイムアタック開始時刻もクライアント側の
  // 値に揃え、同じ合目なら連続正解数だけ大きい方を採用する(端末をまたいでも
  // 進捗が巻き戻らないようにするため)。
  let mergedFujiStation = Number(row.fujiStation) || 0;
  let mergedFujiLegStreak = Number(row.fujiLegStreak) || 0;
  let mergedFujiTimeAttackStartedAt = row.fujiTimeAttackStartedAt || null;
  if (body.fujiStation !== undefined) {
    const bodyFujiStation = Math.max(0, Math.min(10, Math.floor(Number(body.fujiStation)) || 0));
    const rowFujiStation = Number(row.fujiStation) || 0;
    if (bodyFujiStation > rowFujiStation) {
      mergedFujiStation = bodyFujiStation;
      mergedFujiLegStreak = Math.max(0, Math.floor(Number(body.fujiLegStreak)) || 0);
      mergedFujiTimeAttackStartedAt = body.fujiTimeAttackStartedAt ? String(Math.floor(Number(body.fujiTimeAttackStartedAt))) : null;
      set('fuji_station', mergedFujiStation);
      set('fuji_leg_streak', mergedFujiLegStreak);
      set('fuji_time_attack_started_at', mergedFujiTimeAttackStartedAt);
    } else if (bodyFujiStation === rowFujiStation) {
      const bodyLegStreak = Math.max(0, Math.floor(Number(body.fujiLegStreak)) || 0);
      if (bodyLegStreak > (Number(row.fujiLegStreak) || 0)) {
        mergedFujiLegStreak = bodyLegStreak;
        set('fuji_leg_streak', mergedFujiLegStreak);
      }
    }
  }
  ...
  return {
    ...
    completionBonusesAwarded, categoryRanks: mergedCategoryRanks, hpLoginBonusDate: mergedHpLoginBonusDate,
    fujiSummitReached: (body.fujiSummitReached !== undefined) ? (!!row.fujiSummitReached || !!body.fujiSummitReached) : !!row.fujiSummitReached,
    yushaSwordObtained: (body.yushaSwordObtained !== undefined) ? (!!row.yushaSwordObtained || !!body.yushaSwordObtained) : !!row.yushaSwordObtained,
    fujiStation: mergedFujiStation, fujiLegStreak: mergedFujiLegStreak, fujiTimeAttackStartedAt: mergedFujiTimeAttackStartedAt,
