...
function continentBonusForTransition(prevCount, newCount) {
  let bonus = 0;
  for (const c of CONTINENT_DEFS) {
    if (newCount >= c.maxCode && prevCount < c.maxCode) bonus += CONTINENT_BONUS_MP;
  }
  return bonus;
}
...
async function handleSyncPoints(body) {
  ...
  // 世界一周の制覇済みヵ国数(worldCountry)は、レベル10ごとのサイコロで進んだり
  // 戻ったりするクライアント管理の値(hpと同様、サーバー側では直接信用してSETする)。
  // 大陸制覇ボーナスは、直前の保存値から今回の申告値へ大陸の閾値をまたいだ回だけ
  // 付与する(以前はlevelから逆算していたが、サイコロ方式ではlevelから一意に
  // 求まらなくなったため、worldCountry自体の差分で判定する)。
  if (body.worldCountry !== undefined) {
    ...
    continentBonusAwarded = continentBonusForTransition(Number(row.worldCountry) || 0, submittedWorldCountry);
