...
async function handleSyncPoints(body) {
  ...
  // 世界一周2周目(9月〜)のボスを倒して覚えた属性魔法。rareCollectedと同じく
  // 和集合でマージし、周をまたいでも失われない永続記録として扱う。
  if (body.worldSpells !== undefined) {
    const submitted = Array.isArray(body.worldSpells) ? body.worldSpells : [];
    const merged = row.worldSpells.slice();
    submitted.forEach((sid) => { if (!merged.includes(sid)) merged.push(sid); });
    set('world_spells', JSON.stringify(merged));
  }
