...
function worldCountForLevel(level) {
  ...
  return Math.min(WORLD_DATA_LENGTH, Math.floor((level - 100) / 10) + 1);
}
...
async function handleSyncPoints(body) {
  ...
  if (body.level !== undefined && body.exp !== undefined) {
    ...
    continentBonusAwarded = continentBonusForTransition(worldCountForLevel(row.level), worldCountForLevel(clampedLevel));
