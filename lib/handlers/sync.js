...
async function handleSyncPoints(body) {
  ...
  // 宝箱(2026-08-28〜)は、レアキャラ撃破という無償のイベントで手に入るため
  // (鍵の購入・指輪の売却・宝箱を開ける操作とは違いMPが直接動かない)、
  // items/rareCollectedと同じくクライアントを信頼し、各ティアの個数を
  // 現在値との大きい方で更新する(小さい方向には減らさない)。鍵・指輪の個数は
  // ここでは一切書き換えず、専用エンドポイント(buyTreasureKey/sellTreasureRing/
  // openTreasureChest)でサーバー検証のうえ更新する。
  if (body.treasureChests !== undefined && typeof body.treasureChests === 'object' && body.treasureChests !== null) {
    const items = Object.assign({}, row.treasureItems);
    ['bronze', 'silver', 'gold', 'rainbow'].forEach((tier) => {
      const key = 'chest' + tier[0].toUpperCase() + tier.slice(1);
      const submitted = Math.max(0, Math.floor(Number(body.treasureChests[tier])) || 0);
      const current = Number(items[key]) || 0;
      if (submitted > current) items[key] = submitted;
    });
