...
const POINTS_DAILY_CAP_TOTAL_SERVER = 100;
...
// ゾンビ化を治す「ゾンビワクチン」。ゾンビ化状態自体はAKRの祈り/呪いと同じく
// バトル演出用のフラグでクライアント側(localStorage)だけが持っており、
// サーバー側ではMPの消費だけを検証・実行する。
const ZOMBIE_VACCINE_COST_MP = 100;

async function handleZombieVaccine(body) {
  const id = String(body.id || '').trim();
  if (!id) return { ok: false, error: 'missing_id' };
  const row = await findStudent(id);
  if (!row) return { ok: false, error: 'not_found' };
  if (row.points < ZOMBIE_VACCINE_COST_MP) return { ok: false, error: 'insufficient_points' };

  const remaining = row.points - ZOMBIE_VACCINE_COST_MP;
  await sql().query('UPDATE students SET points = $1 WHERE id = $2', [remaining, id]);
  return { ok: true, remainingPoints: remaining };
}

// 刀を研ぐ：使用済みで壊れた「勇者の剣」を、再び1回使えるように直す。500MP。
// 剣を一度も手に入れていない、またはまだ壊れていない(所持数が1のまま)場合は
// 研ぐ必要がないのでエラーを返す。
const SHARPEN_SWORD_COST_MP = 500;

async function handleSharpenSword(body) {
  const id = String(body.id || '').trim();
  if (!id) return { ok: false, error: 'missing_id' };
  const row = await findStudent(id);
  if (!row) return { ok: false, error: 'not_found' };
  if (!row.yushaSwordObtained) return { ok: false, error: 'no_sword' };
  if (row.yushaSwordCount > 0) return { ok: false, error: 'already_usable' };
  if (row.points < SHARPEN_SWORD_COST_MP) return { ok: false, error: 'insufficient_points' };

  const remaining = row.points - SHARPEN_SWORD_COST_MP;
  await sql().query('UPDATE students SET points = $1, yusha_sword_count = 1 WHERE id = $2', [remaining, id]);
  return { ok: true, remainingPoints: remaining, yushaSwordCount: 1 };
}
...
// 命の水：超絶薬草のさらに上位版。5000MPでHPを3000増やせる。
const SEIMEI_MIZU_COST_MP = 5000;
const SEIMEI_MIZU_HP_GAIN = 3000;

async function handleBuySeimeiMizu(body) {
  const id = String(body.id || '').trim();
  if (!id) return { ok: false, error: 'missing_id' };
  const row = await findStudent(id);
  if (!row) return { ok: false, error: 'not_found' };
  if (row.points < SEIMEI_MIZU_COST_MP) return { ok: false, error: 'insufficient_points' };

  const remainingPoints = row.points - SEIMEI_MIZU_COST_MP;
  const newHp = (Number(row.hp) || 0) + SEIMEI_MIZU_HP_GAIN;
  await sql().query('UPDATE students SET points = $1, hp = $2 WHERE id = $3', [remainingPoints, newHp, id]);
  return { ok: true, remainingPoints, hp: newHp };
}
...
async function handleOpenTreasureChest(body) {
  ...
  // 宝箱を開けて得るMPは(指輪の売却と違って)1日の上限100MP(計算50+文章題50)の
  // 対象。以前はここが端末申告のmpGrantをほぼそのまま信用していたため、複数端末で
  // 開ければ端末の台数分だけ上限を超えて稼げてしまっていた(syncPointsと同種の
  // 不具合)。ここでもpoints_today_calc/wordをサーバー側の実際の残り枠として見て、
  // 申告値がどうであれ実際に加算されるMPはその残り枠を超えないようにクランプする
  // (どちらのバケツに数えるかは重要ではないため、便宜的にwordバケツに加える)。
  ...
  const headroom = Math.max(0, POINTS_DAILY_CAP_TOTAL_SERVER - (serverCalc + serverWord));
  const mpGrant = Math.max(0, Math.min(reward.mp, headroom));
  const newTodayWord = serverWord + mpGrant;
  const remainingPoints = row.points + mpGrant;
  const newHp = (Number(row.hp) || 0) + reward.hp;
  await sql().query(
    'UPDATE students SET points = $1, hp = $2, treasure_items = $3, points_date = $4, points_today_calc = $5, points_today_word = $6 WHERE id = $7',
    [remainingPoints, newHp, JSON.stringify(items), today, serverCalc, newTodayWord, id]
  );
...
// スーパーアキラメタル(2026-09-18〜)：漁師AKRと同じく間違えても逃げないレア
// キャラだが、1問間違えるたびにMP-10・HP-10というより重いペナルティを受ける
// (代わりに撃破すると必ず鍵が1つ手に入る)。ポイントの同期(syncPoints)は
// 「サーバー保持値と端末申告値の大きい方」を採用する仕組みのため、そのまま
// state.pointsを減らして送っても減少分がサーバーに反映されない。そのため、
// 購入(handleBuyHerbなど)と同じ「サーバー側で現在値から確定的に引く」方式で
// 専用のペナルティ処理を用意する。
const SUPERAKIRAMETAL_MP_PENALTY = 10;
const SUPERAKIRAMETAL_HP_PENALTY = 10;
async function handleSuperAkirametalPenalty(body) {
  const id = String(body.id || '').trim();
  if (!id) return { ok: false, error: 'missing_id' };
  const row = await findStudent(id);
  if (!row) return { ok: false, error: 'not_found' };

  const remainingPoints = Math.max(0, row.points - SUPERAKIRAMETAL_MP_PENALTY);
  const remainingHp = Math.max(0, row.hp - SUPERAKIRAMETAL_HP_PENALTY);
  await sql().query('UPDATE students SET points = $1, hp = $2 WHERE id = $3', [remainingPoints, remainingHp, id]);
  return { ok: true, points: remainingPoints, hp: remainingHp };
}

module.exports = {
  handleAkrPrayer, AKR_PRAYER_COST_MP,
  handleZombieVaccine, ZOMBIE_VACCINE_COST_MP,
  handleSharpenSword, SHARPEN_SWORD_COST_MP,
  ...
  handleBuySeimeiMizu, SEIMEI_MIZU_COST_MP, SEIMEI_MIZU_HP_GAIN,
