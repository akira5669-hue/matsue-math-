...
const SPELLBOOK_ELEMENTS = ['fire', 'ice', 'thunder'];
const SPELLBOOK_COST_MP = 100;
...
async function handleBuySpellbook(body) {
  ...
  const row = await findStudent(id);
  if (!row) return { ok: false, error: 'not_found' };
  if (row.points < SPELLBOOK_COST_MP) return { ok: false, error: 'insufficient_points' };

  const books = Object.assign({}, row.spellbooks);
  books[element] = (Number(books[element]) || 0) + 1;
  const remainingPoints = row.points - SPELLBOOK_COST_MP;
