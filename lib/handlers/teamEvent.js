...
async function handleTeamEventStatus(body) {
  ...
  // start_dateはまだ先(告知期間中)のことがある。その間はチーム分けの詳細を
  // 見せず、開始日を知らせる告知だけにする(5日夜の最終編成まではチームが
  // 変わる可能性があるため)。
  const started = dateKeyTokyo(new Date()) >= event.start_date;
  ...
  return {
    ...
    myTeam: started ? myTeam : null,
    allTeams: started ? allTeams.map((t) => ({
