...
async function handleTeamEventStatus(body) {
  ...
  const rows = await sql().query(
    `SELECT t.id AS team_id, t.team_name, m.student_id, m.start_level,
            s.name AS student_name, s.grade, s.level AS current_level
  ...
  rows.forEach((row) => {
    ...
    teamMap.get(row.team_id).members.push({
      id: row.student_id, name: row.student_name, grade: row.grade, gained, startLevel: Number(row.start_level) || 0,
    });
  });
