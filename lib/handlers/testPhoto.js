...
const {
  ...
  PROOF_TEST_MP, PROOF_TEST_ALLOWED_GRADES, PROOF_TEST_DEADLINE,
...
async function handleSubmitTestPhoto(body) {
  ...
  if (testType === 'hyakuMasu') {
    timeSeconds = Math.floor(Number(body.timeSeconds));
    if (!Number.isFinite(timeSeconds) || timeSeconds <= 0 || timeSeconds > 5999) {
      return { ok: false, error: 'missing_fields' };
    }
  }
  ...
  if (testType === 'proof') {
    if (!PROOF_TEST_ALLOWED_GRADES.includes(row.grade) && id !== '00001') return { ok: false, error: 'grade_not_allowed' };
    if (dateKeyTokyo(new Date()) > PROOF_TEST_DEADLINE) return { ok: false, error: 'submission_closed' };
  }
  ...
  if (testType === 'proof' && row.lastProofTestMonth === currentMonth) {
    return { ok: false, error: 'already_submitted_this_month' };
  }
  ...
  } else if (testType === 'proof') {
    pointsAwarded = PROOF_TEST_MP;
    set('last_proof_test_month', currentMonth);
  } else {
  ...
  if (testType === 'hyakuMasu') {
    await sql().query(
      'INSERT INTO hyakumasu_times (ts, student_id, name, grade, week_key, time_seconds) VALUES (now(), $1, $2, $3, $4, $5)',
      [id, row.name, row.grade, currentWeek, timeSeconds]
    );
  }
