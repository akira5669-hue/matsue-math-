...
// 個人の記録は最大7年分残す想定(削除処理は行わない。study_reportsは他のログ系
// テーブルと同じく永続保存)。他人に見えるのは常に「今月」だけ(handleStudyReportRankingMonth)
// で、本人の過去の記録だけmonthKey指定で遡って見られるようにする(直近7年まで)。
const STUDY_CALENDAR_MAX_YEARS_BACK = 7;

async function handleStudyCalendar(body) {
  const id = String(body.id || '').trim();
  if (!id) return { ok: false, error: 'missing_id' };
  const currentMonthKey = monthKeyTokyo(new Date());
  let monthKey = String(body.monthKey || '').trim();
  if (!/^\d{4}-\d{2}$/.test(monthKey)) monthKey = currentMonthKey;
  const oldestAllowed = (Number(currentMonthKey.slice(0, 4)) - STUDY_CALENDAR_MAX_YEARS_BACK) + currentMonthKey.slice(4);
  if (monthKey > currentMonthKey) monthKey = currentMonthKey;
  if (monthKey < oldestAllowed) monthKey = oldestAllowed;
