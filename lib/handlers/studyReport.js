...
async function handleStudyReportRanking(body) {
  ...
  const data = await sql().query('SELECT id, grade, study_report_minutes_total FROM students WHERE study_report_minutes_total > 0', []);
  const elementary = [];
  const middle = [];
  for (const d of data) {
    if (!d.id) continue;
    const minutes = Number(d.study_report_minutes_total) || 0;
    if (minutes <= 0) continue;
    const grade = d.grade || '';
    const entry = { id: d.id, minutes, grade };
