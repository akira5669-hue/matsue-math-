(function () {
  ...
  // 光の反射と屈折（中1）：反射の法則(入射角=反射角)、鏡にうつる位置、乱反射、
  // 光の屈折(屈折角と入射角の大小関係)、全反射、プリズムでの分光。
  function genLightReflectionRefraction1() {
    const pat = randInt(0, 12);
    let question, answer, choices, steps;
    if (pat === 0) {
      question = '光が鏡などの面に当たってはね返る現象を何といいますか。';
      answer = '光の反射';
      choices = shuffle(['光の反射', '光の屈折', '光の直進', '全反射']);
      steps = ['光が物体の表面に当たってはね返る現象を光の反射という'];
    } else if (pat === 1) {
      question = '光が反射するとき、入射角と反射角の大きさの関係を表す法則を何といいますか。';
      answer = '光の反射の法則（入射角＝反射角）';
      choices = shuffle(['光の反射の法則（入射角＝反射角）', '光の屈折の法則（入射角＞屈折角）', '全反射の法則', '光の直進の法則']);
      steps = ['光が反射するとき、入射角と反射角は常に等しくなる。これを光の反射の法則という'];
    } else if (pat === 2) {
      const a = randInt(15, 80);
      question = `鏡の面に垂直な線に対して入射角${a}°で光が入射し、反射しました。反射角は何度ですか。`;
      answer = a;
      const wrongs = [90 - a, Math.max(1, a - 10), a + 10].filter(w => w > 0 && w < 90 && w !== a);
      steps = ['光の反射の法則より、反射角は入射角と等しくなる', `反射角 = ${a}°`];
      return { category: 'lightReflectionRefraction1', question, answer, choices: buildChoices(answer, wrongs), steps };
    } else if (pat === 3) {
      const b = randInt(3, 30);
      question = `物体を鏡の面から${b}cmの位置に置きました。鏡にうつって見える物体の位置(像)は、鏡の面から何cmの位置に見えますか。`;
      answer = b;
      const wrongs = [b * 2, Math.max(1, Math.round(b / 2)), b + 3, Math.max(1, b - 3)].filter(w => w !== b);
      steps = ['鏡にうつって見える物体の位置(像)は、鏡をはさんで物体と対称の位置にでき、鏡からの距離は物体と同じになる', `${b}cm`];
      return { category: 'lightReflectionRefraction1', question, answer, choices: buildChoices(answer, wrongs), steps };
    } else if (pat === 4) {
      question = 'でこぼこした面に光が当たると、いろいろな方向に光が反射する現象を何といいますか。';
      answer = '乱反射';
      choices = shuffle(['乱反射', '全反射', '光の屈折', '光の直進']);
      steps = ['表面が凸凹している物体に光が当たると、光がいろいろな方向に反射する。これを乱反射という'];
    } else if (pat === 5) {
      question = '乱反射が起こるおかげで、私たちにはどのような良いことがありますか。';
      answer = '物体を、どの方向からでも見ることができる';
      choices = shuffle(['物体を、どの方向からでも見ることができる', '物体が光を出すようになる', '物体の形が変化する', '物体が鏡のようになる']);
      steps = ['光を反射する物体の表面は完全な平面ではないため乱反射が起こり、いろいろな方向に光が届くので、どの位置からでも物体を見ることができる'];
    } else if (pat === 6) {
      question = '自ら光を出す物体を何といいますか。';
      answer = '光源';
      choices = shuffle(['光源', '光軸', '媒質', '発光体']);
      steps = ['太陽や電灯のように、自ら光を出す物体を光源という'];
    } else if (pat === 7) {
      question = '光が空間をまっすぐに進む性質を何といいますか。';
      answer = '光の直進';
      choices = shuffle(['光の直進', '光の反射', '光の屈折', '光の分散']);
      steps = ['光は空間をまっすぐに進む。これを光の直進という'];
    } else if (pat === 8) {
      question = '光が、空気とガラスのように異なる物質の境界へ進むとき、境界面で光の道筋が折れ曲がる現象を何といいますか。';
      answer = '光の屈折';
      choices = shuffle(['光の屈折', '光の反射', '光の直進', '全反射']);
      steps = ['異なる物質の境界に光が進むとき、境界面で道筋が折れ曲がる現象を光の屈折という'];
    } else if (pat === 9) {
      const material = shuffle(['ガラス', '水'])[0];
      question = `光が空気中から${material}中へ進むとき、屈折角と入射角の大小関係として正しいものはどれですか。`;
      answer = '屈折角の方が入射角より小さい';
      choices = shuffle([answer, '屈折角の方が入射角より大きい', '屈折角と入射角は常に等しい', '屈折角も入射角も常に0°になる']);
      steps = [`光が空気中から${material}のような物質へ進むとき、光は境界面に垂直な線に近づくように折れ曲がるため、屈折角は入射角より小さくなる`];
    } else if (pat === 10) {
      const material = shuffle(['ガラス', '水'])[0];
      question = `光が${material}中から空気中へ進むとき、屈折角と入射角の大小関係として正しいものはどれですか。`;
      answer = '屈折角の方が入射角より大きい';
      choices = shuffle([answer, '屈折角の方が入射角より小さい', '屈折角と入射角は常に等しい', '屈折角も入射角も常に0°になる']);
      steps = [`光が${material}のような物質から空気中へ進むとき、光は境界面に垂直な線から遠ざかるように折れ曲がるため、屈折角は入射角より大きくなる`];
    } else if (pat === 11) {
      question = '光が水やガラスなどから空気中へ進むとき、入射角を大きくしていくと、ある角度をこえたところで光が境界面から外に出られなくなり、すべて反射する現象を何といいますか。';
      answer = '全反射';
      choices = shuffle(['全反射', '乱反射', '光の屈折', '光の直進']);
      steps = ['水やガラスから空気中へ光が進むとき、入射角がある角度(臨界角)より大きくなると、光は境界面を通りぬけずにすべて反射する。これを全反射といい、光ファイバーなどに利用されている'];
    } else {
      question = 'プリズムに太陽の光(白色光)を通すと、光が色によって分かれて見えます。この理由として正しいものはどれですか。';
      answer = '光の色によって、屈折のしかたが異なるから';
      choices = shuffle([answer, '光の色によって、反射のしかたが異なるから', 'プリズムの中で光の色が新しく作られるから', '光の速さは色によって変わることはないから']);
      steps = ['太陽の光には、いろいろな色の光が混ざっている。色によって屈折のしかた(屈折する角度)が異なるため、プリズムを通すと光の色が分かれて見える'];
    }
    return { category: 'lightReflectionRefraction1', question, answer, choices, steps };
  }

  // 凸レンズのはたらき（中1）：光軸・焦点・焦点距離、凸レンズを通る光の3パターン、
  // 実像と虚像、物体の位置による像の大きさ・向きの変化。
  function genConvexLens1() {
    const pat = randInt(0, 12);
    let question, answer, choices, steps;
    if (pat === 0) {
      question = '中央がふくらんでいて、光を集めるはたらきのあるレンズを何といいますか。';
      answer = '凸レンズ';
      choices = shuffle(['凸レンズ', '凹レンズ', '光源', 'プリズム']);
      steps = ['中央が周辺部より厚くふくらんでいるレンズを凸レンズといい、光を集めるはたらきがある'];
    } else if (pat === 1) {
      question = '凸レンズの中心を通り、レンズの面に垂直な直線を何といいますか。';
      answer = '光軸';
      choices = shuffle(['光軸', '焦点', '主軸', '中心線']);
      steps = ['凸レンズの中心を通り、レンズの面に垂直な直線を光軸という'];
    } else if (pat === 2) {
      question = '光軸に平行な光が凸レンズを通ったとき、屈折して1点に集まります。この点を何といいますか。';
      answer = '焦点';
      choices = shuffle(['焦点', '光軸', '虚像', '作用点']);
      steps = ['光軸に平行な光が凸レンズを通って屈折し、集まる点を焦点という。焦点は凸レンズの両側に1つずつある'];
    } else if (pat === 3) {
      question = '凸レンズの中心から焦点までの距離を何といいますか。';
      answer = '焦点距離';
      choices = shuffle(['焦点距離', '光軸の長さ', '像距離', '中心距離']);
      steps = ['凸レンズの中心から焦点までの距離を焦点距離という'];
    } else if (pat === 4) {
      question = '光軸に平行に凸レンズに入った光は、凸レンズを通った後どのように進みますか。';
      answer = '屈折して焦点を通る';
      choices = shuffle(['屈折して焦点を通る', 'そのまま直進する', '屈折して光軸に平行に進む', '反射して戻る']);
      steps = ['光軸に平行に凸レンズに入った光は、凸レンズで屈折して反対側の焦点を通るように進む'];
    } else if (pat === 5) {
      question = '凸レンズの中心を通る光は、凸レンズを通った後どのように進みますか。';
      answer = 'そのまま直進する';
      choices = shuffle(['そのまま直進する', '屈折して焦点を通る', '屈折して光軸に平行に進む', '反射する']);
      steps = ['凸レンズの中心を通る光は、屈折せずにそのまま直進する'];
    } else if (pat === 6) {
      question = '焦点を通って凸レンズに入った光は、凸レンズを通った後どのように進みますか。';
      answer = '屈折して光軸に平行に進む';
      choices = shuffle(['屈折して光軸に平行に進む', 'そのまま直進する', '屈折して焦点を通る', '反射して焦点に戻る']);
      steps = ['焦点を通って凸レンズに入った光は、凸レンズで屈折して光軸に平行に進む'];
    } else if (pat === 7) {
      question = '物体から出た光が凸レンズを通って屈折し、実際に集まってスクリーンなどにうつる像を何といいますか。';
      answer = '実像';
      choices = shuffle(['実像', '虚像', '光源', '焦点']);
      steps = ['物体から出た光が凸レンズで屈折して実際に集まってできる、スクリーンにうつすことができる像を実像という'];
    } else if (pat === 8) {
      question = '凸レンズによってスクリーンにうつる実像は、物体に対して上下左右がどのようになっていますか。';
      answer = '上下左右が逆向き';
      choices = shuffle(['上下左右が逆向き', '上下左右が同じ向き', '上下だけが逆向き', '左右だけが逆向き']);
      steps = ['凸レンズによってスクリーンにうつる実像は、物体に対して上下左右が逆向きになる'];
    } else if (pat === 9) {
      question = '物体を凸レンズの焦点距離の2倍の位置に置いたとき、できる実像の大きさは物体と比べてどうなりますか。';
      answer = '物体と同じ大きさ';
      choices = shuffle(['物体と同じ大きさ', '物体より大きい', '物体より小さい', '実像はできない']);
      steps = ['物体を焦点距離の2倍の位置に置くと、レンズの反対側の焦点距離の2倍の位置に、物体と同じ大きさの実像ができる'];
    } else if (pat === 10) {
      question = '物体を凸レンズの焦点距離の2倍より遠い位置に置いたとき、できる実像の大きさは物体と比べてどうなりますか。';
      answer = '物体より小さい';
      choices = shuffle(['物体より小さい', '物体より大きい', '物体と同じ大きさ', '実像はできない']);
      steps = ['物体を焦点距離の2倍より遠い位置に置くと、焦点距離の2倍の位置と焦点の間に、物体より小さい実像ができる'];
    } else if (pat === 11) {
      question = '物体を凸レンズの焦点距離の2倍の位置と焦点の間に置いたとき、できる実像の大きさは物体と比べてどうなりますか。';
      answer = '物体より大きい';
      choices = shuffle(['物体より大きい', '物体より小さい', '物体と同じ大きさ', '実像はできない']);
      steps = ['物体を焦点距離の2倍の位置と焦点の間に置くと、焦点距離の2倍より遠い位置に、物体より大きい実像ができる'];
    } else {
      question = '物体を凸レンズの焦点の内側(焦点とレンズの間)に置くと、実像はできません。このとき凸レンズを通して見える、物体より大きく物体と同じ向きに見える像を何といいますか。';
      answer = '虚像';
      choices = shuffle(['虚像', '実像', '光源', '焦点']);
      steps = ['物体を焦点の内側に置くと、光が集まらないため実像はできないが、凸レンズを通して見ると、物体より大きく、物体と同じ向きの虚像が見える(虫眼鏡のしくみ)'];
    }
    return { category: 'convexLens1', question, answer, choices, steps };
  }

  // 音の世界（中1）：音源と振動、音を伝える物質、音の速さの計算、
  // 振幅と音の大きさ、振動数と音の高さ。
  function genSound1() {
    const pat = randInt(0, 11);
    let question, answer, choices, steps;
    if (pat === 0) {
      ...
      answer = '音源（発音体）';
      choices = shuffle(['音源（発音体）', '振動体', '共鳴体', '伝導体']);
      steps = ['振動することによって音を出している物体を音源(発音体)という'];
    } else if (pat === 1) {
      question = '音源から出た音が空気中を伝わっていくとき、空気はどのようになっていますか。';
      answer = '振動しながら波のように伝わっていく';
      choices = shuffle(['振動しながら波のように伝わっていく', '空気そのものが音源から耳まで移動する', '音は空気を通らずに伝わる', '空気の温度だけが変化する']);
      steps = ['音源が振動すると、まわりの空気も振動し、その振動が波として次々と伝わっていくことで音が伝わる'];
    } else if (pat === 2) {
      question = '音を伝えることができる物質として正しいものはどれですか。';
      answer = '空気などの気体、水などの液体、金属などの固体';
      choices = shuffle([answer, '空気などの気体だけ', '水などの液体だけ', '金属などの固体だけ']);
      steps = ['音は空気のような気体だけでなく、水などの液体、金属などの固体の中も伝わる'];
    } else if (pat === 3) {
      question = '容器の中の空気をぬいて真空に近づけていくと、容器の中で鳴っているブザーの音の聞こえ方はどうなりますか。';
      answer = 'だんだん聞こえなくなる';
      choices = shuffle(['だんだん聞こえなくなる', '変化しない', 'だんだん大きくなる', '音の高さだけが変わる']);
      steps = ['音を伝える空気がなくなっていくため、真空に近づくほど音は聞こえにくくなる。音は真空中を伝わることができない'];
    } else if (pat === 4) {
      const t = randInt(2, 8);
      const d = 340 * t;
      question = `音の伝わる速さを秒速340mとします。音源から${d}m離れた場所で音を聞くとき、音が聞こえるまでに何秒かかりますか。`;
      answer = t;
      const wrongs = [t + 1, t + 2, Math.max(1, t - 1)];
      steps = [`時間 = 距離 ÷ 速さ = ${d} ÷ 340 = ${t}秒`];
      return { category: 'sound1', question, answer, choices: buildChoices(answer, wrongs), steps };
    } else if (pat === 5) {
      const t = randInt(2, 8);
      const d = 340 * t;
      question = `雷が見えてから${t}秒後に音が聞こえました。音の伝わる速さを秒速340mとすると、雷までの距離は何mですか。`;
      answer = d;
      const wrongs = [d + 340, Math.max(340, d - 340), d * 2];
      steps = [`距離 = 速さ × 時間 = 340 × ${t} = ${d}m`];
      return { category: 'sound1', question, answer, choices: buildChoices(answer, wrongs), steps };
    } else if (pat === 6) {
      question = '花火が見えてから、少し遅れて音が聞こえるのはなぜですか。';
      answer = '光の速さは音の速さよりもはるかに速いから';
      choices = shuffle([answer, '音の速さは光の速さよりもはるかに速いから', '光と音は同時に伝わるから', '花火の音は初めから遅れて出るから']);
      steps = ['光の速さ(秒速約30万km)は、音の速さ(秒速約340m)よりもはるかに速いため、光が届いてから音が遅れて届く'];
    } else if (pat === 7) {
      question = '音源が振動するときの、振動のふれ幅を何といいますか。';
      answer = '振幅';
      choices = shuffle(['振幅', '振動数', '周期', '波長']);
      steps = ['振動している音源の、振動の中心からのふれ幅を振幅という'];
    } else if (pat === 8) {
      question = '弦を強くはじいて振幅を大きくすると、音の大きさはどうなりますか。';
      answer = '大きくなる';
      choices = shuffle(['大きくなる', '小さくなる', '変わらない', '高くなる']);
      steps = ['振幅が大きいほど、音は大きくなる'];
    } else if (pat === 9) {
      question = '音源が1秒間に振動する回数を何といいますか。また、その単位は何ですか。';
      answer = '振動数（単位：ヘルツ、記号Hz）';
      choices = shuffle([answer, '振幅（単位：メートル、記号m）', '周期（単位：秒、記号s）', '音圧（単位：パスカル、記号Pa）']);
      steps = ['音源が1秒間に振動する回数を振動数といい、単位はヘルツ(記号Hz)である'];
    } else if (pat === 10) {
      question = '弦の振動数が多いほど、音の高さはどうなりますか。';
      answer = '高くなる';
      choices = shuffle(['高くなる', '低くなる', '変わらない', '大きくなる']);
      steps = ['振動数が多いほど、音は高くなる'];
    } else {
      question = 'モノコードで、弦の張りを強くしたり、弦の長さを短くしたりすると、音の高さはどうなりますか。';
      answer = '高くなる';
      choices = shuffle(['高くなる', '低くなる', '変わらない', '大きくなる（振幅が変わる）']);
      steps = ['弦の張りを強くする、弦の長さを短くする、弦を細くすると、弦の振動数が多くなり、音は高くなる'];
    }
    return { category: 'sound1', question, answer, choices, steps };
  }

  // 力のはたらきと表し方（中1）：力の3つのはたらき、弾性力・摩擦力・垂直抗力・
  // 重力・磁力、力の単位(ニュートン)とフックの法則、力の3要素、力のつり合い。
  function genForceBasics1() {
    const pat = randInt(0, 12);
    let question, answer, choices, steps;
    if (pat === 0) {
      question = '力には、物体の形を変える、物体の運動の状態を変える、ともう1つのはたらきがあります。それは何ですか。';
      answer = '物体を支える';
      choices = shuffle(['物体を支える', '物体の色を変える', '物体の温度を変える', '物体の質量を変える']);
      steps = ['力のはたらきには、①物体の形を変える ②物体の運動の状態を変える ③物体を支える、の3つがある'];
    } else if (pat === 1) {
      question = '変形した物体が、もとにもどろうとする性質(弾性)によって生じる力を何といいますか。';
      answer = '弾性力（弾性の力）';
      choices = shuffle(['弾性力（弾性の力）', '摩擦力', '垂直抗力', '磁力']);
      steps = ['ばねやゴムのように、変形した物体がもとに戻ろうとする性質を弾性といい、そのときに生じる力を弾性力(弾性の力)という'];
    } else if (pat === 2) {
      question = '接している物体の間で、物体の運動をさまたげる向きにはたらく力を何といいますか。';
      answer = '摩擦力';
      choices = shuffle(['摩擦力', '弾性力', '垂直抗力', '重力']);
      steps = ['接触している物体どうしがこすり合うときに、物体の運動をさまたげる向きにはたらく力を摩擦力という'];
    } else if (pat === 3) {
      question = '物体が机などの面に接しているとき、面が物体を垂直に押し返す力を何といいますか。';
      answer = '垂直抗力';
      choices = shuffle(['垂直抗力', '弾性力', '摩擦力', '重力']);
      steps = ['物体に接した面が、物体を垂直に押し返す力を垂直抗力という'];
    } else if (pat === 4) {
      question = '地球上にある全ての物体が、地球の中心に向かって受けている力を何といいますか。';
      answer = '重力';
      choices = shuffle(['重力', '磁力', '垂直抗力', '弾性力']);
      steps = ['地球上にある全ての物体が、地球の中心に向かって受ける力を重力という'];
    } else if (pat === 5) {
      question = '磁石と鉄などの物質、または磁石と磁石の間にはたらく力を何といいますか。';
      answer = '磁力（磁石の力）';
      choices = shuffle(['磁力（磁石の力）', '電気の力', '重力', '摩擦力']);
      steps = ['磁石と鉄などの物質、磁石どうしの間にはたらく力を磁力(磁石の力)という'];
    } else if (pat === 6) {
      question = '磁石のN極とN極を近づけると、どうなりますか。';
      answer = '退け合う（反発し合う）';
      choices = shuffle(['退け合う（反発し合う）', '引き合う', '何も起こらない', 'くっついて離れなくなる']);
      steps = ['同じ極(N極とN極、S極とS極)どうしは退け合い、異なる極(N極とS極)どうしは引き合う'];
    } else if (pat === 7) {
      const m = randInt(1, 9) * 100;
      const n = m / 100;
      question = `質量${m}gの物体にはたらく重力の大きさは、約何Nですか。ただし、100gの物体にはたらく重力の大きさを1Nとします。`;
      answer = `約${n}N`;
      const wrongCands = [`約${n + 1}N`, `約${Math.max(1, n - 1)}N`, `約${n * 100}N`];
      steps = [`100gの物体にはたらく重力が1Nなので、${m}gでは ${m} ÷ 100 = ${n}N`];
      return { category: 'forceBasics1', question, answer, choices: buildChoicesFromList(answer, wrongCands), steps };
    } else if (pat === 8) {
      question = 'ばねののびは、ばねを引く力の大きさに比例するという法則を何といいますか。';
      answer = 'フックの法則';
      choices = shuffle(['フックの法則', '反射の法則', '分離の法則', '慣性の法則']);
      steps = ['ばねののびが、ばねに加えた力の大きさに比例するという関係をフックの法則という'];
    } else if (pat === 9) {
      const baseCm = randInt(1, 3);
      const force = randInt(2, 6);
      const nobi = baseCm * force;
      question = `あるばねに1Nの力を加えると、ばねは${baseCm}cmのびました。このばねに${force}Nの力を加えると、のびは何cmになりますか。ただし、ばねののびは力の大きさに比例するものとします。`;
      answer = nobi;
      const wrongs = [nobi + baseCm, Math.max(1, nobi - baseCm), force + baseCm].filter(w => w !== nobi);
      steps = ['フックの法則より、ばねののびは力の大きさに比例する', `${baseCm} × ${force} = ${nobi}cm`];
      return { category: 'forceBasics1', question, answer, choices: buildChoices(answer, wrongs), steps };
    } else if (pat === 10) {
      question = '力を矢印で表すとき、矢印の「長さ」「向き」「始点」は、それぞれ力の何を表していますか。';
      answer = '力の大きさ・力の向き・作用点';
      choices = shuffle(['力の大きさ・力の向き・作用点', '力の種類・力の速さ・力の温度', '力の重さ・力の色・力の形', '力の質量・力の体積・力の密度']);
      steps = ['力は、大きさ・向き・作用点(力がはたらく点)の3つの要素で表され、矢印の長さが力の大きさ、矢印の向きが力の向き、矢印の始点が作用点を表す'];
    } else if (pat === 11) {
      question = '月面上では、同じ物体にはたらく重力の大きさは地球上と比べてどうなりますか。また、物体の質量はどうなりますか。';
      answer = '重力の大きさは地球上の約1/6になるが、質量は変化しない';
      choices = shuffle([answer, '重力の大きさも質量も約1/6になる', '重力の大きさは変化しないが、質量は約1/6になる', '重力の大きさも質量も変化しない']);
      steps = ['重力の大きさは場所によって変わり、月面上では地球上の約1/6になる。一方、物質そのものの量である質量は、場所が変わっても変化しない'];
    } else {
      question = '1つの物体にはたらく2つの力がつり合うための条件として、正しいものはどれですか。';
      answer = '2力は一直線上にあり、大きさが等しく、向きが反対である';
      choices = shuffle([answer, '2力は一直線上にあり、大きさが等しく、向きも同じである', '2力の大きさが異なり、向きが反対である', '2力の作用点が同じ点にある']);
      steps = ['1つの物体にはたらく2つの力がつり合う条件は、①2力が一直線上にある ②2力の大きさが等しい ③2力の向きが反対である、の3つである'];
    }
    return { category: 'forceBasics1', question, answer, choices, steps };
  }

  // 火山の活動と火成岩（中1）：マグマと溶岩、マグマのねばりけと火山の形・噴火の
  // ようす、火山灰と鉱物、火成岩(火山岩・深成岩)のつくりと種類。
  function genVolcano1() {
    const pat = randInt(0, 12);
    let question, answer, choices, steps;
    if (pat === 0) {
      question = '地下にある岩石などが高熱でとけたものを何といいますか。';
      answer = 'マグマ';
      choices = shuffle(['マグマ', '溶岩', '火山灰', '鉱物']);
      steps = ['地球内部の熱によって地下の岩石などがとけたものをマグマという'];
    } else if (pat === 1) {
      question = 'マグマが地表に流れ出たもの、または、それが冷えて固まったものを何といいますか。';
      answer = '溶岩';
      choices = shuffle(['溶岩', 'マグマ', '火山灰', '軽石']);
      steps = ['マグマが地表に噴き出したもの、あるいはそれが冷え固まったものを溶岩という'];
    } else if (pat === 2) {
      question = 'マグマのねばりけが弱い(流動性が高い)場合、火山の形はどのようになりやすいですか。';
      answer = '傾斜がゆるやかな形（盾状）の火山';
      choices = shuffle([answer, '盛り上がったドーム状の形の火山', '円錐に近い形の火山', '常に平らな形の火山']);
      steps = ['マグマのねばりけが弱いほど、溶岩がさらさらと流れやすく、傾斜のゆるやかな形の火山になる'];
    } else if (pat === 3) {
      question = 'マグマのねばりけが強い場合、噴火のようすと溶岩の色はどのようになりやすいですか。';
      answer = '激しく爆発的な噴火になり、溶岩は白っぽい';
      choices = shuffle([answer, 'おだやかな噴火になり、溶岩は白っぽい', '激しく爆発的な噴火になり、溶岩は黒っぽい', 'おだやかな噴火になり、溶岩は黒っぽい']);
      steps = ['マグマのねばりけが強いほど、噴火は激しく爆発的になりやすく、溶岩は流れにくく白っぽい色になる'];
    } else if (pat === 4) {
      question = '火山が噴火したときにふき出される、細かい粒でできた固体の噴出物を何といいますか。';
      answer = '火山灰';
      choices = shuffle(['火山灰', '火山ガス', '溶岩', '鉱物']);
      steps = ['火山の噴火によってふき出される、直径2mm以下の細かい粒を火山灰という'];
    } else if (pat === 5) {
      question = '火山灰の粒の形には、どのような特徴がありますか。';
      answer = '角ばった形をしている';
      choices = shuffle(['角ばった形をしている', '丸みを帯びている', '必ず球形をしている', '必ず板状である']);
      steps = ['火山灰の粒は、流水によって運ばれてできたものではないため、角ばった形をしていることが多い'];
    } else if (pat === 6) {
      const colorless = ['セキエイ', 'チョウ石'];
      const colored = ['クロウンモ', 'カクセン石', 'キ石', 'カンラン石'];
      const correct = shuffle(colorless)[0];
      const wrongPool = shuffle(colored);
      question = '次の鉱物のうち、無色鉱物はどれですか。';
      answer = correct;
      choices = shuffle([correct, ...wrongPool.slice(0, 3)]);
      steps = ['セキエイやチョウ石のように、白色〜無色で決まった色をもたない鉱物を無色鉱物、クロウンモ・カクセン石・キ石・カンラン石のように黒色〜暗緑色などの色をもつ鉱物を有色鉱物という'];
    } else if (pat === 7) {
      question = 'マグマが冷えて固まってできた岩石を何といいますか。';
      answer = '火成岩';
      choices = shuffle(['火成岩', '堆積岩', '深成岩', '火山岩']);
      steps = ['マグマが冷えて固まってできた岩石を火成岩といい、火山岩と深成岩に分けられる'];
    } else if (pat === 8) {
      question = '地表や地表近くで、マグマが急に冷えて固まってできた火成岩を何といいますか。';
      answer = '火山岩';
      choices = shuffle(['火山岩', '深成岩', '堆積岩', '鉱物']);
      steps = ['地表や地表付近でマグマが急速に冷えて固まった火成岩を火山岩という'];
    } else if (pat === 9) {
      question = '地下深くで、マグマが長い時間をかけてゆっくり冷えて固まってできた火成岩を何といいますか。';
      answer = '深成岩';
      choices = shuffle(['深成岩', '火山岩', '堆積岩', '鉱物']);
      steps = ['地下深くでマグマが長い時間をかけてゆっくり冷えて固まった火成岩を深成岩という'];
    } else if (pat === 10) {
      question = '石基の中に、比較的大きな結晶(斑晶)が散らばって見える火山岩のつくりを何といいますか。';
      answer = '斑状組織';
      choices = shuffle(['斑状組織', '等粒状組織', '結晶質組織', '層状組織']);
      steps = ['マグマが地表付近で急速に冷えると、大きな結晶(斑晶)が細かい粒(石基)の間に散らばった斑状組織になる'];
    } else if (pat === 11) {
      question = 'ほぼ同じ大きさの鉱物の結晶が、すき間なく組み合わさってできている深成岩のつくりを何といいますか。';
      answer = '等粒状組織';
      choices = shuffle(['等粒状組織', '斑状組織', '結晶質組織', '層状組織']);
      steps = ['マグマが地下深くでゆっくり冷えると、大きな鉱物の結晶がすき間なく組み合わさった等粒状組織になる'];
    } else {
      question = '火山岩のうち、色が最も黒っぽい岩石はどれですか。';
      answer = '玄武岩';
      choices = shuffle(['玄武岩', '安山岩', '流紋岩', '花こう岩']);
      steps = ['火山岩は、有色鉱物を多く含むほど黒っぽくなる。玄武岩(黒っぽい)・安山岩(中間)・流紋岩(白っぽい)の順に色が白くなっていく'];
    }
    return { category: 'volcano1', question, answer, choices, steps };
  }

  // 地震のゆれの伝わり方（中1）：震源・震央・震度・マグニチュード、P波とS波、
  // 初期微動と主要動、初期微動継続時間の計算、プレートと地震の発生のしくみ。
  function genEarthquake1() {
    const pat = randInt(0, 13);
    let question, answer, choices, steps;
    if (pat === 0) {
      question = '地震が発生した地下の場所を何といいますか。';
      answer = '震源';
      choices = shuffle(['震源', '震央', '震度', '断層']);
      steps = ['地震が発生した地下の場所を震源という'];
    } else if (pat === 1) {
      question = '震源の真上にある、地表の地点を何といいますか。';
      answer = '震央';
      choices = shuffle(['震央', '震源', '震度', 'プレート']);
      steps = ['震源の真上にある地表の地点を震央という'];
    } else if (pat === 2) {
      question = 'ある地点での地震のゆれの大きさを表す震度は、何階級に分けられていますか。';
      answer = '10階級（0、1、2、3、4、5弱、5強、6弱、6強、7）';
      choices = shuffle([answer, '7階級（0〜6）', '5階級（1〜5）', '100階級']);
      steps = ['震度は0、1、2、3、4、5弱、5強、6弱、6強、7の10階級に分けられている'];
    } else if (pat === 3) {
      question = '地震そのものの規模(エネルギーの大きさ)を表す量を何といいますか。また、その記号は何ですか。';
      answer = 'マグニチュード（記号：M）';
      choices = shuffle([answer, '震度（記号：I）', '初期微動継続時間（記号：T）', '震源距離（記号：D）']);
      steps = ['地震の規模(エネルギーの大きさ)はマグニチュード(記号M)で表される。マグニチュードが1大きくなるとエネルギーは約32倍になる'];
    } else if (pat === 4) {
      question = '地震のゆれのうち、伝わる速さが速く、先に届いて小さいゆれ(初期微動)を起こす波を何といいますか。';
      answer = 'P波';
      choices = shuffle(['P波', 'S波', '津波', '主要動']);
      steps = ['伝わる速さが速く、震源から先に届いて初期微動を起こす波をP波(Primary wave)という'];
    } else if (pat === 5) {
      question = '地震のゆれのうち、P波より遅れて届き、大きなゆれ(主要動)を起こす波を何といいますか。';
      answer = 'S波';
      choices = shuffle(['S波', 'P波', '津波', '初期微動']);
      steps = ['P波より伝わる速さが遅く、後から届いて主要動を起こす波をS波(Secondary wave)という'];
    } else if (pat === 6) {
      question = 'P波が届いてから、S波が届くまでの間に起こる小さなゆれを何といいますか。';
      answer = '初期微動';
      choices = shuffle(['初期微動', '主要動', '余震', '前震']);
      steps = ['P波が届いてからS波が届くまでの間に起こる小さなゆれを初期微動という'];
    } else if (pat === 7) {
      question = 'P波が届いてからS波が届くまでの時間を何といいますか。';
      answer = '初期微動継続時間';
      choices = shuffle(['初期微動継続時間', '主要動継続時間', '震源距離', 'マグニチュード']);
      steps = ['P波が届いてからS波が届くまでの時間を初期微動継続時間という'];
    } else if (pat === 8) {
      question = '震源からの距離と、初期微動継続時間には、どのような関係がありますか。';
      answer = '震源からの距離が長いほど、初期微動継続時間は長くなる（比例する）';
      choices = shuffle([answer, '震源からの距離が長いほど、初期微動継続時間は短くなる', '震源からの距離と初期微動継続時間は無関係である', '震源からの距離が長いほど、初期微動継続時間は0に近づく']);
      steps = ['P波とS波の伝わる速さの差により、震源からの距離が長いほどP波とS波の到着時刻の差(初期微動継続時間)は長くなる。この関係はほぼ比例する'];
    } else if (pat === 9) {
      const d = randInt(3, 10) * 8;
      const tp = d / 8;
      const ts = d / 4;
      const duration = ts - tp;
      question = `ある地点は震源から${d}km離れています。P波の伝わる速さを秒速8km、S波の伝わる速さを秒速4kmとすると、この地点での初期微動継続時間は何秒ですか。`;
      answer = duration;
      const wrongs = [duration + 2, Math.max(1, duration - 2), ts].filter(w => w !== duration);
      steps = [`P波が届く時刻 = ${d} ÷ 8 = ${tp}秒後`, `S波が届く時刻 = ${d} ÷ 4 = ${ts}秒後`, `初期微動継続時間 = ${ts} − ${tp} = ${duration}秒`];
      return { category: 'earthquake1', question, answer, choices: buildChoices(answer, wrongs), steps };
    } else if (pat === 10) {
      const dur = randInt(2, 9);
      const d = dur * 8;
      question = `ある地点での初期微動継続時間は${dur}秒でした。P波の伝わる速さを秒速8km、S波の伝わる速さを秒速4kmとすると、この地点の震源からの距離は何kmですか。`;
      answer = d;
      const wrongs = [d + 8, Math.max(8, d - 8), d * 2].filter(w => w !== d);
      steps = ['初期微動継続時間は震源からの距離に比例する', `震源からの距離 = 8 × ${dur} = ${d}km`];
      return { category: 'earthquake1', question, answer, choices: buildChoices(answer, wrongs), steps };
    } else if (pat === 11) {
      question = '地球の表面をおおっている、厚さ100kmほどの岩盤を何といいますか。';
      answer = 'プレート';
      choices = shuffle(['プレート', '断層', 'マグマ', '震源域']);
      steps = ['地球の表面は、プレートとよばれる厚さ100kmほどの岩盤でおおわれている。日本付近には4つのプレートが集まっている'];
    } else if (pat === 12) {
      question = '海洋プレートが大陸プレートの下に沈みこみ、引きずりこまれた大陸プレートの先端部が、限界に達してはね上がることで起こる大きな地震を何といいますか。';
      answer = '海溝型地震（プレートの境界で起こる地震）';
      choices = shuffle([answer, '内陸型地震（活断層による地震）', '火山性地震', '誘発地震']);
      steps = ['海洋プレートに引きずりこまれた大陸プレートの先端部がひずみに耐えきれずはね上がることで起こる大きな地震を、海溝型地震(プレートの境界で起こる地震)という'];
    } else {
      question = '海底で地震が起こったときに、海水が押し上げられて発生する波を何といいますか。';
      answer = '津波';
      choices = shuffle(['津波', '高潮', '液状化現象', '初期微動']);
      steps = ['海底で地震が発生し、海底が急激に上下することで海水が押し上げられて発生する波を津波という'];
    }
    return { category: 'earthquake1', question, answer, choices, steps };
  }

  // 地層のなり立ちと化石（中1）：風化・侵食・運搬・堆積、堆積岩(れき岩・砂岩・
  // 泥岩・凝灰岩・石灰岩・チャート)、化石(示相化石・示準化石)、しゅう曲・断層。
  function genStrataFossil1() {
    const pat = randInt(0, 14);
    let question, answer, choices, steps;
    if (pat === 0) {
      question = '気温の変化や雨水のはたらきなどによって、岩石の表面がもろくなっていく現象を何といいますか。';
      answer = '風化';
      choices = shuffle(['風化', '侵食', '運搬', '堆積']);
      steps = ['気温の変化や雨水のはたらきなどにより、長い間に岩石の表面がもろくなっていく現象を風化という'];
    } else if (pat === 1) {
      question = '流れる水が、地表の岩石や地面をけずるはたらきを何といいますか。';
      answer = '侵食';
      choices = shuffle(['侵食', '運搬', '堆積', '風化']);
      steps = ['流れる水が、岩石や地面をけずるはたらきを侵食という'];
    } else if (pat === 2) {
      question = '侵食された土砂を、流れる水が下流へ運ぶはたらきを何といいますか。';
      answer = '運搬';
      choices = shuffle(['運搬', '侵食', '堆積', '風化']);
      steps = ['流水によって土砂が運ばれるはたらきを運搬という'];
    } else if (pat === 3) {
      question = '運搬された土砂が、流れが緩やかなところや河口・海底などに積もることを何といいますか。';
      answer = '堆積';
      choices = shuffle(['堆積', '運搬', '侵食', '風化']);
      steps = ['土砂が流れの緩やかなところや河口・海底などに積もることを堆積という'];
    } else if (pat === 4) {
      question = '堆積が長い年月をかけて繰り返され、しま模様に積み重なった土砂や岩石の層を何といいますか。';
      answer = '地層';
      choices = shuffle(['地層', '露頭', '柱状図', '堆積岩']);
      steps = ['土砂が長い年月をかけて層状に積み重なったものを地層という'];
    } else if (pat === 5) {
      question = '地層や岩石の一部が、がけなどで地表に現れているところを何といいますか。';
      answer = '露頭';
      choices = shuffle(['露頭', '地層', '柱状図', '堆積岩']);
      steps = ['地層や岩石が、がけなどで地表に現れているところを露頭という'];
    } else if (pat === 6) {
      const sizes = [
        { name: 'れき岩', desc: '粒の直径が2mm以上' },
        { name: '砂岩', desc: '粒の直径が2mm〜1/16mmくらい' },
        { name: '泥岩', desc: '粒の直径が1/16mm以下' },
      ];
      const pick = shuffle(sizes)[0];
      question = `堆積岩のうち、${pick.desc}の粒が堆積してできた岩石を何といいますか。`;
      answer = pick.name;
      choices = shuffle([...sizes.map(s => s.name), '凝灰岩']);
      steps = ['れき岩は直径2mm以上、砂岩は2mm〜1/16mm、泥岩は1/16mm以下の粒でできている(凝灰岩は粒の大きさではなく、火山灰などの火山噴出物からできる堆積岩)'];
    } else if (pat === 7) {
      question = 'れき岩や砂岩に含まれる粒は、角が取れて丸みを帯びていることが多いです。それはなぜですか。';
      answer = '流水によって運ばれる間に、角がけずられたから';
      choices = shuffle([answer, 'マグマが冷え固まるときに丸くなるから', '火山の噴火によって丸くなるから', '地震のゆれによって丸くなるから']);
      steps = ['れき岩や砂岩をつくる粒は、流水によって運ばれる間に、川底や他の粒とぶつかって角がけずられ、丸みを帯びる'];
    } else if (pat === 8) {
      question = '火山灰や軽石など、火山噴出物が堆積して固まってできた堆積岩を何といいますか。';
      answer = '凝灰岩';
      choices = shuffle(['凝灰岩', '石灰岩', 'チャート', '泥岩']);
      steps = ['火山灰などの火山噴出物が堆積して固まった岩石を凝灰岩という。凝灰岩の層が見つかると、その地層が堆積した当時、近くで火山の噴火があったことがわかる'];
    } else if (pat === 9) {
      question = '貝がらやサンゴなど、炭酸カルシウムを主成分とする生物の遺骸が堆積してできた石灰岩に、うすい塩酸をかけると、どうなりますか。';
      answer = 'とけて二酸化炭素が発生する';
      choices = shuffle([answer, 'とけて酸素が発生する', '変化しない（とけない）', 'とけて水素が発生する']);
      steps = ['石灰岩は炭酸カルシウムを主成分とし、うすい塩酸をかけると反応してとけ、二酸化炭素が発生する'];
    } else if (pat === 10) {
      question = '放散虫など、二酸化ケイ素を主成分とする生物の遺骸が堆積してできた、非常にかたいチャートに、うすい塩酸をかけると、どうなりますか。';
      answer = 'ほとんど変化しない（とけない）';
      choices = shuffle([answer, 'とけて二酸化炭素が発生する', 'とけて酸素が発生する', 'とけて水素が発生する']);
      steps = ['チャートは二酸化ケイ素を主成分とする非常にかたい岩石で、うすい塩酸をかけても反応しない'];
    } else if (pat === 11) {
      question = '大昔の生物の死がいや、生活のあと(巣穴・足あとなど)が地層の中に残ったものを何といいますか。';
      answer = '化石';
      choices = shuffle(['化石', '鉱物', '示準化石', '示相化石']);
      steps = ['大昔の生物の死がいや生活のあとが地層中に残ったものを化石という'];
    } else if (pat === 12) {
      question = 'あたたかくて浅い海にすむ生物であるサンゴの化石が地層から見つかった場合、その地層が堆積した当時の環境について何がわかりますか。';
      answer = 'あたたかくて浅い海であったこと';
      choices = shuffle([answer, '寒冷な深い海であったこと', '陸上の森林であったこと', '高い山であったこと']);
      steps = ['このように、地層が堆積した当時の環境を推定できる化石を示相化石という。サンゴの化石は、あたたかくて浅い海であったことを示す'];
    } else if (pat === 13) {
      question = 'ある限られた時代にだけ栄え、広い範囲にすんでいた生物の化石は、地層が堆積した何を知る手がかりになりますか。';
      answer = '地層が堆積した年代';
      choices = shuffle([answer, '地層が堆積した当時の環境', '地層の厚さ', '地層をつくる粒の大きさ']);
      steps = ['限られた期間に広い範囲で栄え、その後絶滅した生物の化石を示準化石といい、地層が堆積した年代を知る手がかりになる'];
    } else {
      question = '地層が下から上へと積み重なってできているとき、地層の新旧の関係として正しいものはどれですか。(大地の大きな変動がない場合)';
      answer = '下にある層ほど古く、上にある層ほど新しい';
      choices = shuffle([answer, '上にある層ほど古く、下にある層ほど新しい', 'どの層も同じ年代である', '地層から年代はわからない']);
  ...
  function renderTeamEventBanner_() {
    ...
    if (!teamEventCache.started) {
      ...
      els.teamEventBannerText.textContent =
        ...
        `1チームおよそ30人で、期間中の経験値の上がり方を競います。最終的なチーム編成は開始直前に確定するので、新しく登録した人・最近また遊び始めた人もどこかのチームに入ります。\n` +
    ...
    els.teamEventBannerText.textContent =
      `🤝 チーム対抗経験値バトル開催中！（${ev.startDate}〜${ev.endDate}）\n` +
      `1チームおよそ30人で、期間中にどれだけ経験値を上げられるかを競います。\n` +
      `チームの経験値上昇量の順位に応じて、1位10000MP・2位7000MP・3位5000MP・4位3000MP・5位1000MPが、チーム内で経験値を上げた量に比例して分配されます（6位以下は分配なし、1つも上げていない人には分配されません）。\n` +
